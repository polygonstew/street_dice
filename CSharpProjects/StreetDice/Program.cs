...
string scoreC = $"""
 ...
 /$$__  $$          \x1b[0m$-----------------\x1b[32m$
| $$  \__/         | Player 1: {$"\x1b[33m$\x1b[32m{p1cash}"}  |
|  $$$$$$          | Player 2: {$"\x1b[33m$\x1b[32m{p2cash}"}  |
 \____  $$          -----------------
 /$$  \ $$           {$"\x1b[0mChose to: \x1b[0m{matchFade}\x1b[5m"}
...
Console.WriteLine($"                           Press any key to continue..."); // <-- suppose to flash, but not working. Going to remove the \x1b] additions to these Ready.Key lines
Console.ReadKey();
Console.Clear();
titleCard();
Console.WriteLine("\n--- Player 1 Rolls ---");
...
Console.WriteLine("\nAny key for Player 2 to place their bet...");
Console.ReadKey();
int p2PlaceBet = 0;
while (true)
{
    Console.WriteLine("\nPlayer 2, place your bet! (enter a number)");
    string p2BetInput = Console.ReadLine();
    if (int.TryParse(p2BetInput, out p2PlaceBet) && p2PlaceBet > 0 && p2PlaceBet <= 100)
    {
        break;
    }
    else
